...
public class OVRPlayerController : MonoBehaviour
{
	...
	public virtual void UpdateMovement()
	{
		...
		// Modificacion segun Manager Devide
		bool moveForward = Input.GetKey(md.kb_Up);
		bool moveLeft = Input.GetKey(md.kb_Left);
		bool moveRight = Input.GetKey(md.kb_Right);
		bool moveBack = Input.GetKey(md.kb_Down);
		bool moveUp = Input.GetKey(md.kb_Y_Up);
		bool moveDown = Input.GetKey(md.kb_Y_Down);
		...
		// Run - Modificacion Manager Device
		if (dpad_move || Input.GetKey(md.kb_Run))
			moveInfluence *= 2.0f;

		if (dpad_move || Input.GetKey(md.kb_Slow))
			moveInfluence *= 0.15f;
		...
		if (moveUp){
			MoveThrottle += ort * (transform.lossyScale.x * moveInfluence * BackAndSideDampen * Vector3.up);
